...
const allImages = Array.from({ length: 9 }, (_, i) => ({
  ...
  alt: `PES Event ${i + 1}`,
  category: i < 4 ? 'OJT Party' : i < 7 ? 'Skill Camp' : 'Success Stories',
}));

const tabs = ['All', 'OJT Party', 'Skill Camp', 'Success Stories'];
      ...
      <section className="navy-gradient pt-32 pb-14 md:pt-40 md:pb-20">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-white/70 text-base">
            OJT Parties • Skill Camps at Pawna Lake • Success Celebrations
          </motion.p>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto px-4 flex gap-2 overflow-x-auto py-3">
          {tabs.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-all ${activeTab === tab ? 'bg-navy text-white' : 'bg-bg-light text-navy hover:bg-navy/10'}`}>
              {tab}
            </button>
          ))}
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          {activeTab === 'OJT Party' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center mb-6">
              <h2 className="font-serif text-navy text-xl md:text-2xl font-bold">Celebrating Our Students&apos; Journey into Industry</h2>
              <p className="text-text-muted text-sm mt-1">Every OJT party marks a milestone — students earning while learning!</p>
            </motion.div>
          )}
          {activeTab === 'Skill Camp' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center mb-6">
              <h2 className="font-serif text-navy text-xl md:text-2xl font-bold">Skill Camp — Pawna Lake</h2>
              <p className="text-text-muted text-sm mt-1">Where Learning Meets Adventure. Team-building and leadership at its finest.</p>
            </motion.div>
          )}
