...
const filters = ['All', '3-Year Programs', 'Short Courses', 'Postgraduate'];
...
export default function CoursesPage() {
  ...
  const filtered = courses.filter((c) => {
    ...
    if (active === '3-Year Programs') return c.duration === '3 Years';
    if (active === 'Short Courses') return c.duration === '6 Months' || c.duration === '1 Month';
    if (active === 'Postgraduate') return c.duration === '2 Years';
    return true;
  });
    ...
    <>
      ...
      <section className="navy-gradient pt-32 pb-14 md:pt-40 md:pb-20">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          >
            Industry-designed. Practically-delivered. Career-guaranteed.
          </motion.p>
        </div>
      </section>
          ...
          {filters.map((f) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-all ${
                active === f ? 'bg-navy text-white' : 'bg-bg-light text-navy hover:bg-navy/10'
              }`}
            >
              {f}
            </button>
      ...
      <section className="py-10 md:py-14 bg-bg-light">
        <div className="max-w-7xl mx-auto px-4">
          ...
          {filtered.length === 0 && (
            <p className="text-center text-text-muted py-10">No courses match this filter.</p>
          )}
        </div>
      </section>
    </>
