...
const WA_BASE = 'https://wa.me/917666676358?text=';
...
interface FormData {
  ...
  course: string; stream: string; percentage: string;
}
...
function buildWAMessage(f: FormData) {
  return encodeURIComponent(
    `Hi, I want to apply for AEDP at TJ College Kirkee!\n\nName: ${f.name}\nPhone: ${f.phone}\nEmail: ${f.email}\nCity: ${f.city}\nCourse: ${f.course}\nStream: ${f.stream}\n12th %: ${f.percentage}%\n\nPlease guide me on the admission process.`
  );
}
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const result = await submitToGoogleSheets({
        ...
        city: form.city, course: form.course, stream: form.stream, percentage: form.percentage,
      }, 'tj-college');
      if (result?.error) toast.error('Could not save your details. Please call +91 76666 76358.');
    } catch {
      toast.error('Could not save your details. Please call +91 76666 76358.');
    }
    setLoading(false);
    setSubmitted(true);
      ...
      <section ref={ref} id="apply-form" className="py-14 md:py-20" style={{ background: 'linear-gradient(135deg,#071232,#0B1F5C)' }}>
        <div className="max-w-xl mx-auto px-4 text-center">
          <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 200 }}>
            ...
            <p className="text-white/70 text-sm mb-6">WhatsApp opened to connect you directly. Our team will reply within 24 hours.</p>
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <motion.form onSubmit={handleSubmit}
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-6">
            ...
            <a href={`${WA_BASE}Hi%2C+I+want+to+know+about+AEDP+admissions+at+TJ+College+Kirkee`}
