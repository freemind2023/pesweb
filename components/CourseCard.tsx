  ...
  return (
    ...
    >
      <div className="relative aspect-[16/9] overflow-hidden">
        <Image src={course.image} alt={course.name} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" />
        ...
        <div className="absolute top-3 left-3 flex gap-2 flex-wrap">
          ...
          {course.dubaiPlacement && (
            <span className="px-2.5 py-1 bg-accent text-white text-xs font-bold rounded-full">🌍 Dubai</span>
          )}
        </div>
      </div>

      <div className="p-4 md:p-5">
        <h3 className="font-serif text-navy font-bold text-lg mb-1 group-hover:text-gold transition-colors">{course.name}</h3>
        <p className="text-text-muted text-sm mb-3 line-clamp-2">{course.description}</p>

        {detailed && (
          <div className="mb-3 flex flex-wrap gap-1.5">
            {course.subjects.slice(0, 4).map((subject) => (
              <span key={subject} className="px-2 py-0.5 bg-bg-light text-navy text-xs rounded-full font-medium">{subject}</span>
            ))}
          </div>
        )}
        ...
        <div className="flex gap-2">
          <Link href={`/courses/${course.slug}`}
            ...
            {tr.knowMore} <ArrowRight size={14} />
          </Link>
