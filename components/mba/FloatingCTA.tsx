...
export default function FloatingCTA({ onApply }: { onApply: () => void }) {
  ...
  useEffect(() => {
    const onScroll = () => {
      if (!barDismissed) setShowBar(window.scrollY > 500);
    };
        ...
        {showBar && !barDismissed && (
          <motion.div
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white font-black text-sm truncate">Practical MBA — Applications Open!</p>
              <p className="text-white/50 text-[10px] truncate">18 Months OJT with Stipend — Limited seats</p>
            </div>
            <button
              onClick={onApply}
            ...
            <a href={WA} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl font-bold text-white text-xs flex-shrink-0 hidden sm:flex"
              style={{ background: '#25D366' }}>
