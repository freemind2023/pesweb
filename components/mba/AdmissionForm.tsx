...
const INITIAL: FormData = { name: '', phone: '', email: '', city: '', stream: '', graduation: '' };
    ...
    <section ref={ref} id="apply-form" className="py-14 md:py-20" style={{ background: 'linear-gradient(135deg,#071232,#0B1F5C)' }}>
      <div className="max-w-4xl mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <h2 className="text-white text-3xl md:text-4xl font-black">Apply for Practical MBA</h2>
        ...
        <motion.form onSubmit={handleSubmit}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            ...
            <Field label="Graduation Stream *" id="stream" error={errors.stream}>
              <select id="stream" value={form.stream} onChange={(e) => set('stream', e.target.value)} className={inputCls(errors.stream)}>
                <option value="">Select stream</option>
                {STREAMS.map((s) => <option key={s}>{s}</option>)}
              </select>
            </Field>
            <Field label="Graduation Details (optional)" id="graduation">
              <input id="graduation" type="text" placeholder="e.g. B.Com, Final Year" value={form.graduation}
                onChange={(e) => set('graduation', e.target.value)} className={inputCls()} />
            </Field>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-6">
            ...
            <a href={`${WA_BASE}Hi%2C+I+want+to+know+about+the+Practical+MBA+program+at+Practical+EduSkills`}
