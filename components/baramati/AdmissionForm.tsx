...
const WA_BASE = 'https://wa.me/919689348709?text=';
const STREAMS = ['Commerce', 'Arts', 'Science', 'Other'];

interface FormData { name: string; phone: string; email: string; city: string; stream: string; percentage: string; }
const INITIAL: FormData = { name: '', phone: '', email: '', city: '', stream: '', percentage: '' };

function buildWAMessage(f: FormData) {
  return encodeURIComponent(
    `Hi, I want to apply for Practical B.COM (Fintech & Digital Accounting) at College of Practical Commerce, Baramati!\n\nName: ${f.name}\nPhone: ${f.phone}\nEmail: ${f.email}\nCity: ${f.city}\nStream: ${f.stream}\n12th %: ${f.percentage}%\n\nPlease guide me.`
  );
}
...
const AdmissionForm = forwardRef<HTMLDivElement>(function AdmissionForm(_, ref) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (result?.error) toast.error('Could not save your details. Please call +91 96893 48709.');
    } catch { toast.error('Could not save your details. Please call +91 96893 48709.'); }
    setLoading(false);
    setSubmitted(true);
      ...
      <section ref={ref} id="apply-form" className="py-14 md:py-20" style={{ background: 'linear-gradient(135deg,#071232,#0B1F5C)' }}>
        <div className="max-w-xl mx-auto px-4 text-center">
          <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 200 }}>
            ...
            <p className="text-white/70 text-sm mb-6">WhatsApp opened. Our Baramati team will reply within 24 hours.</p>
            <a href={`${WA_BASE}${buildWAMessage(form)}`} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-white text-sm" style={{ background: '#25D366' }}>
      ...
      <div className="max-w-4xl mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[#0B1F5C] text-xs font-bold mb-3"
            style={{ background: '#F5B400' }}>2025–26 Admissions Open · Baramati</span>
          <h2 className="text-white text-3xl md:text-4xl font-black">Apply Now — It&apos;s Free</h2>
          <p className="text-white/60 text-sm mt-2">Fill in your details and our Baramati counsellor will reach you within 24 hours.</p>
        </motion.div>
        ...
        <motion.form onSubmit={handleSubmit}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            ...
            <Field label="City *" id="city" error={errors.city}>
              <input id="city" type="text" placeholder="Baramati / Pune / …" value={form.city}
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-6">
            ...
            <a href={`${WA_BASE}Hi%2C+I+want+to+know+about+Practical+BCom+at+College+of+Practical+Commerce+Baramati`}
          ...
          <p className="text-gray-400 text-[10px] text-center mt-3">By submitting, you agree to be contacted by our admission counsellors. No spam.</p>
        </motion.form>
      </div>
    </section>
  );
